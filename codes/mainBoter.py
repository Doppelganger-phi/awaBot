#encoding=utf-8
from const import *
import websocket, ssl, requests, re, threading, sys, os, init, my, math, datetime, sqlite3, time, traceback
...
def gett(text: str):
    a = text.find("text=")
    b = text.find(", pronunciation=",a)
    return(text[a+5:b])
class grasss(threading.Thread):
    def __init__(self, text):
        threading.Thread.__init__(self)
        self.text = text
    def run(self):
        try:
            cnt = random.randrange(3,8)
            for _ in range(cnt):
                self.text = gett(str(trans.translate(self.text,dest=random.choice(languages))))
            self.text = "&#8205;" + gett(str(trans.translate(self.text,dest='zh-cn')))
            chat.sendMsg(self.text)
        except (RuntimeError, ReadTimeout) as e:
            chat.sendMsg(f"被玩坏了，呜呜呜……\n```\n{e}")
...
def colorPic() -> str:
    def Piccolor() -> None:
        ...
        title = setu["title"]
        author = setu["author"]
        utes = requests.get(url).content
        gyazo_url = my.imgupload(utes)
        chat.sendMsg(f"{gyazo_url}\n原URL：{url}\n标题：{title}\n标签：{','.join(tags)}\n作者：{author}")
    if sysList[0]:
        threading.Thread(target=Piccolor).start()
        return ' hso'
    else:
        return " 害，别惦记你那涩涩了。"
LINE.update({"涩图": colorPic})
...
# 日志日志
def logs(text: str, nick: str, time_, hash_=None, trip=None, color=None,uType=None):
    time_ = int(time.time()*1000) if time_ == None else int(time_)
    text = text.replace('</script>','<\/script>')
    command = f"INSERT INTO Message (chatroom,channel,color,nick,text,time,trip,uType,hash) VALUES (?,?,?,?,?,?,?,?,?)"
    cursor.execute(command,('hack.chat',channel,color,nick,text,time_,trip,uType,hash_))
    con.commit()

def msgGot(chat, msg: str, sender: str, senderTrip: str, result: dict):
    global prime_index,tmp_result,userData
    rans = random.randint(1, 134)
    this_turn = f"{sender}：{msg[:1024]}"
    command = msg[:6]

    logs(msg, sender, result.get('time',None), userHash[sender], senderTrip, userColor[sender], result.get('uType',None))
    if sender in [nick, "ModBot"]: return

    if not senderTrip in whiteList:
        frisked = frisk(hash_ := userHash[sender], 0.9+len(msg)/256)
        if frisked == "warn":
    ...
    if msg[0] == PREFIX:
        ...
        if command == "hash ": chat.sendMsg(hashByName(namePure(msg[6:])))
            ...
            if (color := userColor.get(namePure(msg[6:]))) is not None:
                if color != None: chat.sendMsg(color)
            ...
            if senderTrip:
                ...
                writeJson("../files/userData.json", userData)
            ...
            try:
                digit = abs(int(msg[6:19]))
                chat.sendMsg(getPrime(digit))
            except ValueError:
                tmp_result.clear()
                prime_index = 0
                chat.sendMsg("请输入一个***整数***啊啊啊啊(￢_￢)")
        elif command == "gras ":
            try:
                textt = msg[6:]
                grass(textt)
            except: chat.sendMsg("QAQ")
        elif command == 'tran ':
            try:
                texx = msg[6:]
                def send_translate(text: str, language: str):
                    chat.sendMsg("&#8205;"+gett(str(trans.translate(text,dest=language))))
                if (dstlan := texx.split()[0]) not in languages:
                    threading.Thread(target=send_translate,args=(texx,'zh-cn')).start()
                else:
                    threading.Thread(target=send_translate,args=(texx[texx.find(" ")+1:],dstlan)).start()
            except: chat.sendMsg("QAQ")
        elif command == "rand ":
            try:
                digit = int(msg[6:])
            ...
            if len(array) < 3: chat.sendMsg(f"命令错误，请使用`{PREFIX}repl 提问 回答`的格式(‾◡◝)")
                ...
                chat.sendMsg(f"添加成功(☆▽☆)")
                writeJson("../files/answer.json", answer)
    ...
    elif (msg == "开始u" or len(unos[1]) == 8) and not unos[0]:
        def _start_uno():
            if len(unos[1]) == 8:
                chat.sendMsg('人数达到上限，游戏自动开始！')
            unos[0] = True
            initialize_card()
            for i in range(len(unos[1])):
                ...
                chat.whisper(unos[1][i], f"这是你的牌：{'，'.join(sort_uno(playerCard))}")
        ...
        if card == "check":
            return chat.whisper(sender, f"现在牌面上的牌是=={unos[5]}==，这是你的牌：{'，'.join(sort_uno(unos[2][id_]))}")
        elif card == '.':
            addCard = random.choice(unos[3])
            unos[3].remove(addCard)
            if addCard[0] == unos[5][0] or addCard[1:] == unos[5][1:] or unos[5] == '变色':
                ...
                if addCard[1:] == '禁':
                    ...
                    chat.sendMsg(f'`{sender}`补到了{addCard}并将其打出，`{unos[1][nextid]}`跳过1轮，轮到`{unos[4]}`！')
                elif addCard[1:] == '+2':
                    unos[4] = unos[1][next2id_]
                    no_card(2)
                    for i in range(2):
                    ...
                    chat.sendMsg(f'`{sender}`补到了=={addCard}==并将其打出，`{unos[1][nextid]}`加2张，轮到`{unos[4]}`！')
                    chat.whisper(unos[1][nextid], f"你新增了2张牌，这是你现在的牌：{'，'.join(sort_uno(unos[2][nextid]))}。")
                elif addCard[1:] == '转向':
                    unos[1].reverse()
                    unos[2].reverse()
                    unos[4] = unos[1][(-id_)%len(unos[1])]
                    id_ = (-id_-1)%len(unos[1])
                    chat.sendMsg(f'`{sender}`补到了{addCard}并将其打出，==顺序转换==，轮到`{unos[4]}`！')
                else:
                    unos[4] = unos[1][nextid]
                    chat.sendMsg(f'`{sender}`补到了=={addCard}==并将其打出，轮到`{unos[4]}`！')
            else:
                unos[4] = unos[1][nextid]
                unos[2][id_].append(addCard)
                chat.sendMsg(f'`{sender}`补了一张牌，轮到`{unos[4]}`！')
                chat.whisper(sender, f"你新增了1张牌，这是你现在的牌： {'，'.join(sort_uno(unos[2][id_]))}。")
            return
            ...
            chat.whisper(unos[1][nextid], f"你新增了4张牌，这是你现在的牌：{'，'.join(sort_uno(unos[2][nextid]))}。")
        elif card == "变色":
            if len(msgList) < 3:
                return chat.sendMsg("缺少参数！")
        ...
        # 小黑屋是不值得学习的！
            ...
            if command == "0addw ":
                ...
                if (ind:=msg[6:]) == "0":
                    ...
                    with open("../files/design.json", encoding="utf8") as f:
                        global designs
                        designs = json.load(f)
...
def join(chat, joiner: str, color: str, result: dict):
    ...
    userColor[joiner] = color if color else None
    userHash[joiner], userTrip[joiner] = hash_, trip
    logs(f"{joiner} joined", "*", result.get('time',None), hash_, trip, userColor[joiner], result.get('uType',None))
...
def onSet(chat, result: dict):
    ...
    for i in result["users"]:
        ...
        userColor[nick_] = i["color"] if i["color"] else None
        ...
        if names := data.get(i["hash"]):
            ...
            data[i["hash"]] = [nick_]
    writeJson(FILENAME, data)
    logs(f'Users online: {", ".join(result["nicks"])}','*',result['time'])
...
def changeColor(chat, result:dict):
    '''{'nick': str, 'trip': str, 'uType': 'user', 
        ...
        'channel': str, 'time': int}'''
    userColor[result["nick"]] = result["color"] if result["color"] else None
def leave(chat, leaver: str, time: int):
    chat.onlineUsers.remove(leaver)
    logs(f"{leaver} left", "*", time)
...
def whispered(chat, from_: str, msg: str, result: dict):
    ...
    if result["channel"] != channel: chat.restarts()
    elif msg[0] == PREFIX:
        if command == "left ":
            if len(lis := msg.split()) < 3:
...
def emote(chat, sender: str, msg: str, time: int):
    full = f"*：{msg}"
    logs(msg, "*", time, userHash[sender], userTrip[sender])
    allMsg.append(full)
    if not userTrip[sender] in whiteList:
        ...
        if frisked == "warn":
            ...
            records[hash_]["warned"] = False
class HackChat:
    def __init__(self, channel: str, nick: str, passwd: str, color: str="f1ad9d"):
        global first
        self.nick = nick
        self.channel = channel
        my.renew_tor_ip(ControlPort)
        self.ws = websocket.create_connection("wss://hack.chat/chat-ws", 
            sslopt={"cert_reqs": ssl.CERT_NONE},
            http_proxy_host="127.0.0.1", 
            http_proxy_port=SocksPort, 
            proxy_type="socks5")
        if first:
            for func in [kick_check,self._clock,self._person_control]:
    ...
    def restarts(self):
        try:
            self.ws.close()
            self.__init__(channel, nick, passwd, color)
            self.run()
        except:
            restarth()
    def run(self):
        ...
        try:
            while True:
                ...
                if cmd == "info" and result["text"] == "You have been denied access to that channel and have been moved somewhere else. Retry later or wait for a mod to move you.":
                    passwd = "your_authtrip_password"
                    self.restarts()
                if cmd == "warn" and result["text"] == "Nickname taken":
                    nick += "_"+str(random.randint(0,999))
                    self.restarts()
                if (not sysList[2]) or (sysList[2] and result.get("trip") in whiteList): 
                    # 接收到消息！
                    if cmd == "chat": msgGot(self, result["text"], rnick, result.get('trip',None), result)
                    # 有新人来！
                    elif cmd == "onlineAdd": join(self, rnick, result.get('color', None), result)
                    # 有人离开……
                    elif cmd == "onlineRemove": leave(self, rnick, result.get('time',None))
                    # 收到私信！
                    elif result.get("type") == "whisper":
                        if not isinstance((froms := result["from"]), int) and froms != nick:
                            whispered(self, result["from"], "".join(result["text"].split(":")[1:]), result)
                    # 更换颜色（色色达咩）
                    elif cmd == "updateUser": changeColor(self, result)
                    elif cmd == "emote": emote(self, result["nick"], result["text"], result.get('time',None))
                    elif cmd == "info": logs(result['text'], '*', result.get('time',None))
                    # 话痨过头被服务器娘教训啦——
                    elif cmd == "warn":
                        if "blocked" not in result["text"] and "Wait a moment" not in result["text"]:
                            print(result["text"])
                        else:
                            self.restarts()
                    # 当然要用最好的状态迎接开始啦！
                    ...
                    if cmd == "chat": logs(result["text"], rnick, result.get('time',None), userHash[rnick], result.get('trip',None), userColor[rnick], result.get('uType',None))
                    elif cmd == "emote": logs(result["text"], "*", result.get('time',None), userHash[rnick], userTrip[rnick])
                    elif cmd == "onlineAdd":
                        self.onlineUsers.append(rnick)
                        roloc = result.get('color',None)
                        userColor[rnick] = roloc if roloc else None
                        userHash[rnick], userTrip[rnick] = result["hash"], result.get("trip",None)
                        logs(f"{rnick} joined", "*", result.get('time',None), result['hash'], result.get("trip",None), userColor[rnick], result.get('uType',None))
                    elif cmd == "onlineRemove": leave(self, rnick, result.get('time',None))
                    elif cmd == "updateUser": changeColor(self, result)
                    elif cmd == "info" and result.get("type") != "whisper": logs(result['text'], '*', result.get('time',None))
                    elif cmd == "onlineSet": onSet(self, result)
                    elif cmd == "warn":
                        if "blocked" not in result["text"] and "Wait a moment" not in result["text"]:
                            print(result["text"])
        ...
        # 坏心眼……
            ...
            try:
                #with open(f"../files/{time.time()}.txt", "w", encoding="utf8") as f:
